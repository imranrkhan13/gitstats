// CompareView.jsx — GitStatus
// Changes vs original:
//  • Removed Heatmap / contribGrid (no longer in data — was fake)
//  • Commits tab now aligns two variable-length monthlyCommits arrays by month key
//  • Streak section in commits tab shows real activeDays calendar instead of heatmap
//  • Streak sub-label updated to "within 90d window"
...
import { Spinner, ScoreRing, ChartTooltip, LangBar } from './Atoms.jsx'
import {
  XIcon, ChartIcon, TrophyIcon, CheckIcon, StarIcon, ForkIcon, RepoIcon,
  UsersIcon, FireIcon, CodeIcon, AwardIcon, TrendIcon, ShareIcon,
  IssueIcon, EyeIcon, CompareIcon
} from './Icons.jsx'
import { ShareCard } from './ShareCard.jsx'
import { BRAND } from '../lib/brand.js'
import { calcWinner } from '../lib/winner.js'
import { VSBattle } from './VSBattle.jsx'

function WinBadge({ text }) {
  return (
    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: 9, fontWeight: 800, padding: '3px 8px', borderRadius: 20, background: 'var(--br)', color: '#f5c842', letterSpacing: '0.05em', textTransform: 'uppercase' }}>
      <TrophyIcon size={9} color="#f5c842" />{text}
    </span>
  )
}

function MetricDuel({ label, icon, v1, v2, fmtFn, u1, u2, higherBetter = true }) {
  const total = (v1 + v2) || 1, pct1 = Math.round((v1 / total) * 100)
  const win1 = higherBetter ? v1 > v2 : v1 < v2, win2 = higherBetter ? v2 > v1 : v2 < v1, tie = v1 === v2
  return (
...
// ── Align two variable-length monthlyCommits arrays by month ──────────────────
function alignCommits(mc1, mc2, login1, login2) {
  const map = {}
  mc1.forEach(m => { map[m.key] = { month: m.month, [login1]: m.commits, [login2]: 0 } })
  mc2.forEach(m => {
    if (map[m.key]) map[m.key][login2] = m.commits
    else map[m.key] = { month: m.month, [login1]: 0, [login2]: m.commits }
  })
  return Object.values(map).sort((a, b) => a.month.localeCompare(b.month))
}

// ── Mini 28-day calendar (real events) ───────────────────────────────────────
function MiniCalendar({ activeDays, login }) {
  ...
  return (
    <div>
      <div style={{ fontSize: 10, color: 'var(--text4)', marginBottom: 6 }}>Last 28 days · {login}</div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7,1fr)', gap: 2 }}>
        {last28.map((ds, i) => (
          <div key={i} title={ds} style={{
            aspectRatio: '1', borderRadius: 2,
            background: activeSet.has(ds) ? 'var(--br2)' : 'var(--bg2)',
            border: ds === today ? '1.5px solid var(--br)' : '1px solid transparent',
          }} />
        ))}
      </div>
    </div>
...
export function CompareView({ data1, onClose }) {
  ...
  const tabSt = id => ({
    ...
    background: tab === id ? 'var(--br)' : 'transparent',
  ...
  return (
    <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--r2)', marginBottom: 16, overflow: 'hidden', boxShadow: 'var(--shadow2)' }}>
      ...
      <div style={{ padding: '18px 24px', borderBottom: '1px solid var(--border)', background: 'linear-gradient(90deg,var(--bg) 0%,var(--bg2) 100%)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 10 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 12, color: 'var(--text3)', marginTop: 1 }}>Stats · Repos · Languages · Skill Radar · Activity</div>
          </div>
        </div>
        <div style={{ display: 'flex', gap: 8 }}>
          {data2 && (
            <button onClick={() => setShowShareCard(true)} style={{ height: 34, padding: '0 14px', border: 'none', background: 'var(--br)', color: '#fff', borderRadius: 'var(--r)', fontSize: 12, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6, fontFamily: 'Inter,sans-serif' }}>
              <ShareIcon size={13} color="#fff" />Share Comparison
            </button>
          )}
      ...
      {data2 && (() => {
        const u2 = data2.user
        const alignedCommits = alignCommits(data1.monthlyCommits, data2.monthlyCommits, u1.login, u2.login)
        return (
          <div className="animate-fade">
            {/* Side-by-side comparison */}
            <div style={{ padding: '20px 24px 0' }}>
              <VSBattle data1={data1} data2={data2} />
            </div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                ...
                <div>
                  <div style={{ fontSize: 15, fontWeight: 800, color: winner === 'tie' ? 'var(--text)' : '#f5f0e8', lineHeight: 1.2 }}>
                    {winner === 'tie' ? "It's a tie! Evenly matched" : `${winner === 'user1' ? u1.name || u1.login : u2.name || u2.login} takes the lead`}
                  </div>
                  <div style={{ fontSize: 12, color: winner === 'tie' ? 'var(--text3)' : 'rgba(255,255,255,0.45)', marginTop: 3 }}>
                    {winner === 'tie'
                      ? `${wins1}–${wins2} across ${METRICS.length} metrics · for fun only`
                      : `${winner === 'user1' ? wins1 : wins2}–${winner === 'user1' ? wins2 : wins1} wins · share below`}
                  </div>
                </div>
              </div>
              <div style={{ display: 'flex', gap: 8 }}>
            ...
            <div style={{ padding: '10px 24px 0', borderBottom: '1px solid var(--border)', display: 'flex', gap: 4, overflowX: 'auto' }}>
              {[['overview', 'Overview'], ['repos', 'Repos'], ['languages', 'Languages'], ['radar', 'Radar'], ['activity', 'Activity']].map(([id, label]) => (
            ...
            <div style={{ padding: '22px 24px' }}>
                      ...
                      <div key={u.login} style={{ background: iw ? 'linear-gradient(135deg,#2a1208,#3d1e10)' : 'var(--bg)', border: `1.5px solid ${iw ? 'var(--br2)' : 'var(--border)'}`, borderRadius: 'var(--r2)', padding: '20px', textAlign: 'center', position: 'relative', transition: 'all 0.3s' }}>
                        ...
                        <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 10 }}>
                          {u.avatar_url
                            ...
                            : <div style={{ width: 56, height: 56, borderRadius: '50%', background: 'var(--br)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20, fontWeight: 800, color: '#f5ddb0' }}>{(u.name || u.login)[0].toUpperCase()}</div>}
                        </div>
                    ...
                    return (
                      <>
                        <div style={{ background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: 'var(--r)', padding: '18px', marginBottom: 18 }}>
                          <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--text3)', textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 16 }}>Language Overlap</div>
                    ...
                    <div style={{ display: 'flex', gap: 14 }}>
                      {[{ u: u1, col: 'var(--br)' }, { u: u2, col: 'var(--br4)' }].map(({ u, col }) => (
                        <div key={u.login} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                          ...
                          <div><div style={{ fontSize: 12, fontWeight: 700, color: 'var(--text)' }}>{u.login}</div></div>
                        </div>
                      ))}
                    </div>
                    <div style={{ marginLeft: 'auto', fontSize: 11, color: 'var(--text3)' }}>6 skill dimensions</div>
                  </div>
              ...
              {tab === 'activity' && (
                <div className="animate-fade">
                  {/* Commit chart — aligned by month */}
                  {alignedCommits.length > 0 ? (
                    <div style={{ background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: 'var(--r)', padding: '16px 18px', marginBottom: 16 }}>
                      <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--text3)', textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 4 }}>Commit Activity — Real Events Only (~90d)</div>
                      ...
                      <ResponsiveContainer width="100%" height={180}>
                        <BarChart data={alignedCommits} margin={{ top: 4, right: 4, bottom: 0, left: -20 }}>
                    ...
                    <div style={{ background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: 'var(--r)', padding: '24px', marginBottom: 16, textAlign: 'center', color: 'var(--text4)', fontSize: 13 }}>
                      No commit events found in the last 90 days for either user.
                    </div>
                  )}
                  ...
                  <div className="rg2" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 16 }}>
                    {[{ d: data1, u: u1 }, { d: data2, u: u2 }].map(({ d, u }) => (
                      <div key={u.login} style={{ background: 'linear-gradient(140deg,#2a1208 0%,#1a0a04 100%)', borderRadius: 'var(--r)', padding: '16px', border: '1px solid rgba(255,255,255,0.07)' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
                          {u.avatar_url && <img src={u.avatar_url} alt="" style={{ width: 24, height: 24, borderRadius: '50%', border: '1.5px solid rgba(255,255,255,0.2)' }} />}
                          <span style={{ fontSize: 13, fontWeight: 700, color: '#fff' }}>{u.login}</span>
                        ...
                        <div className="rg2" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginTop: 12 }}>
                          ...
                          <div style={{ background: 'rgba(255,255,255,0.07)', borderRadius: 8, padding: '10px 12px' }}>
                            <div style={{ fontSize: 22, fontWeight: 900, color: '#f5c842', letterSpacing: '-0.03em' }}>{d.longestStreak}</div>
                            <div style={{ fontSize: 9, color: 'rgba(255,255,255,0.35)', textTransform: 'uppercase', letterSpacing: '0.05em', marginTop: 3 }}>Longest (90d)</div>
                          </div>
                        </div>
                      </div>
                    ))}
