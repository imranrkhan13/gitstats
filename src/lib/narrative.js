...
export function buildStory(data) {
  ...
  return {
    ...
    spotlight: spotlightRepo
      ? `Favorite project: ${spotlightRepo.name}${spotlightRepo.stars ? ` — ${spotlightRepo.stars.toLocaleString()} stars` : ''}.${spotlightRepo.desc ? `\n"${spotlightRepo.desc}"` : ''}`
      : null,
    ...
    biggestCodebase: (() => {
      ...
      return biggest && biggest.size > 0 ? `${biggest.name} is the biggest by repo size — ${Math.round(biggest.size / 1024) || 1}MB.` : null
    })(),
    ...
    mostUnderrated: (() => {
      ...
      return best ? `${best.name} — solid by the numbers, quiet on stars (${best.stars}).` : null
    })(),
    ...
    longestMaintained: (() => {
      ...
      return years >= 2 ? `${longest.name} has been maintained for ${years} years — still getting pushes.` : null
    })(),
  }
}
