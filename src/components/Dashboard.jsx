...
function AIPanel({ data }) {
  ...
  const buildPrompt = (type) => {
    ...
    if (type === 'roast') return `You are a witty, savage but good-natured tech roast comedian. Roast this GitHub developer in 3-4 punchy sentences. Be funny and specific, reference their actual stats. No disclaimers, just the roast.
...
Languages: ${langs}
Top repo: ${topRepo?.name || 'none'} (${topRepo?.stars || 0} stars) — ${topRepo?.desc || 'no description'}
...
Member since: ${u.created_at?.slice(0, 4)}`
  ...
  const run = async (type) => {
    ...
    setMode(type); setText(''); setLoading(true)
    ...
    try {
      for (const provider of PROVIDERS) {
        try {
          ...
          if (result.ok) { setText(result.text); return }
    ...
    <div style={{ maxWidth: 980, margin: '0 auto', padding: '0 20px 80px' }}>
      ...
      <div className="gs-toptoolbar" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '18px 0 20px', gap: 12, flexWrap: 'wrap' }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          <button onClick={onShare} style={{ height: 38, padding: '0 18px', border: 'none', background: 'var(--br)', color: '#fff', borderRadius: 'var(--r)', fontSize: 13, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 7, fontFamily: 'Inter,sans-serif', boxShadow: '0 2px 8px rgba(61,32,16,0.25)', transition: 'background 0.2s' }}
