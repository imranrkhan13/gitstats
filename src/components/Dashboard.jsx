// Dashboard.jsx — GitStatus
// New vs original:
//  • Streak mini-calendar now uses REAL activeDays from events (not fake pattern)
//  • PR count, review count, issue count surfaced in Activity tab
//  • "Activity Breakdown" card with pie + day-of-week bar
//  • "Most Active Time" badge in streak card
//  • Repo timeline chart in Activity tab
//  • "Recently Active Repos" section in Repos tab
//  • Real vs estimated commit months marked with dot indicator
...
import {
  ...
  ActivityIcon, CompareIcon, InfoIcon, GridIcon, ListIcon, ZapIcon, ClockIcon
} from './Icons.jsx'
import { BRAND } from '../lib/brand.js'

const ACT_COLORS = { commit: 'var(--br2)', pr: 'var(--blue)', review: '#8b5cf6', create: 'var(--green2)', star: 'var(--amber)', fork: 'var(--purple)', issue: 'var(--red)', other: 'var(--text3)' }
const ACT_LABELS = { commit: 'Commit', pr: 'PR', review: 'Review', create: 'Created', star: 'Starred', fork: 'Forked', issue: 'Issue', other: 'Activity' }
...
function ActivityBreakdownCard({ eventStats }) {
  const { eventBreakdown, dayActivity, prCount, reviewCount, issueCount, pushCount, totalCommits } = eventStats
  const total = eventBreakdown.reduce((a, b) => a + b.value, 0)

  const statPills = [
    { label: 'Push events', value: pushCount, color: 'var(--br2)' },
    { label: 'Commits pushed', value: totalCommits, color: 'var(--br3)' },
    { label: 'Pull Requests', value: prCount, color: 'var(--blue)' },
    { label: 'Code Reviews', value: reviewCount, color: '#8b5cf6' },
    { label: 'Issues', value: issueCount, color: 'var(--red)' },
  ].filter(s => s.value > 0)

  return (
    <Card style={{ marginBottom: 14 }}>
      <SectionLabel>Activity Breakdown (last ~90 days)</SectionLabel>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
        {/* Left — event type pie */}
        <div>
          <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--text3)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 10 }}>Event Types</div>
          {total === 0 ? (
            <p style={{ fontSize: 13, color: 'var(--text4)', fontStyle: 'italic' }}>No public events found</p>
          ) : (
            <>
              <ResponsiveContainer width="100%" height={140}>
                <PieChart>
                  <Pie data={eventBreakdown} cx="50%" cy="50%" innerRadius={38} outerRadius={62} paddingAngle={2} dataKey="value">
        ...
        {/* Right — day of week bar */}
        <div>
          <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--text3)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 10 }}>Activity by Day</div>
          <ResponsiveContainer width="100%" height={140}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: 10 }}>
            {statPills.map(s => (
              <div key={s.label} style={{ padding: '4px 10px', borderRadius: 20, background: 'var(--bg2)', border: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: 6 }}>
...
// ── Real streak mini-calendar (uses activeDays from events) ──────────────────
function StreakCard({ streak, longestStreak, activeDays, lastActive, eventStats }) {
  // Show last 28 days as a grid using real active days
    ...
    <Card style={{ background: 'linear-gradient(140deg,#2a1208 0%,#1a0a04 100%)', border: 'none' }}>
      ...
      {eventStats && (
        <div style={{ display: 'flex', gap: 8, marginBottom: 14, flexWrap: 'wrap' }}>
          <div style={{ background: 'rgba(255,255,255,0.07)', borderRadius: 9, padding: '8px 12px', flex: 1, minWidth: 100 }}>
            <div style={{ fontSize: 9, color: 'rgba(255,255,255,0.35)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 4 }}>Most active day</div>
            <div style={{ fontSize: 14, fontWeight: 800, color: '#fb923c' }}>{eventStats.mostActiveDay}</div>
          </div>
          <div style={{ background: 'rgba(255,255,255,0.07)', borderRadius: 9, padding: '8px 12px', flex: 1, minWidth: 100 }}>
            <div style={{ fontSize: 9, color: 'rgba(255,255,255,0.35)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 4 }}>Codes mostly</div>
            <div style={{ fontSize: 14, fontWeight: 800, color: '#fb923c' }}>{eventStats.mostActiveHour}</div>
          </div>
          {lastActive && (
            <div style={{ background: 'rgba(255,255,255,0.07)', borderRadius: 9, padding: '8px 12px', flex: 1, minWidth: 100 }}>
              <div style={{ fontSize: 9, color: 'rgba(255,255,255,0.35)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 4 }}>Last active</div>
              <div style={{ fontSize: 13, fontWeight: 700, color: 'rgba(255,255,255,0.7)' }}>{lastActive}</div>
            </div>
          )}
        </div>
      )}

      <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.28)', marginBottom: 8, textTransform: 'uppercase', letterSpacing: '0.06em' }}>
        Last 28 days {activeDays?.length > 0 ? '(real data)' : ''}
      </div>
      ...
      {activeDays?.length === 0 && (
        <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.25)', marginTop: 8, fontStyle: 'italic' }}>
          No public events found in the last 90 days
        </div>
      )}
    </Card>
  )
...
export function Dashboard({ data, onShare, onCompare }) {
  ...
  const {
    ...
    topByStars, recentlyActive, monthlyCommits, contribGrid, streak, longestStreak,
    score, activity, repoTypes, memberYears, avgStars, stack, radarData,
    spotlightRepo, devType, githubAge, influence, topTopics, estContributions,
    eventStats, repoTimeline, activeDays, lastActive,
  } = data
  ...
  return (
    <div style={{ maxWidth: 980, margin: '0 auto', padding: '0 20px 80px' }}>
        ...
        <>
          ...
          <Card style={{ marginBottom: 14 }} className="animate-fade-up d3">
            ...
            <div style={{ fontSize: 11, color: 'var(--text4)', marginTop: 8, fontStyle: 'italic' }}>
              Heatmap is estimated (GitHub REST API doesn't expose full contribution data)
            </div>
          </Card>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1.15fr 0.85fr', gap: 12, marginBottom: 14 }}>
            <Card className="animate-fade-up d4">
              <SectionLabel right={
                <div style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 11, color: 'var(--text4)' }}>
                  <div style={{ width: 7, height: 7, borderRadius: '50%', background: 'var(--br2)' }} /> real
                  <div style={{ width: 7, height: 7, borderRadius: '50%', background: 'var(--border2)', marginLeft: 6 }} /> est.
                </div>
              }>Monthly Commits (12 months)</SectionLabel>
                ...
                <BarChart data={monthlyCommits} margin={{ top: 4, right: 4, bottom: 0, left: -22 }}>
                  ...
                  <Bar dataKey="commits" radius={[3, 3, 0, 0]} maxBarSize={26}>
                    {monthlyCommits.map((m, i) => (
                      <Cell key={i} fill={
                        !m.isReal ? 'var(--border2)' :
                          i === monthlyCommits.length - 1 ? 'var(--br)' :
                            i >= monthlyCommits.length - 3 ? 'var(--br2)' : 'var(--br3)'
                      } />
                    ))}
                  </Bar>
                </BarChart>
      ...
      {activeTab === 'repos' && (
        <>
          {/* Recently active repos (separate from top by stars) */}
          {recentlyActive?.length > 0 && (
            <Card style={{ marginBottom: 14 }} className="animate-fade-up">
              <SectionLabel right={<span style={{ fontSize: 11, color: 'var(--text4)' }}>sorted by last push</span>}>Recently Active</SectionLabel>
                ...
                {recentlyActive.map(r => (
                  <a key={r.name} href={r.url || `https://github.com/${user.login}/${r.name}`} target="_blank" rel="noopener noreferrer" style={{ textDecoration: 'none' }}>
                    <div style={{ background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: 'var(--r)', padding: '12px 14px', transition: 'border-color 0.15s,box-shadow 0.15s' }}
                      ...
                      {r.desc && <p style={{ fontSize: 11, color: 'var(--text3)', marginTop: 4, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{r.desc}</p>}
      ...
      {activeTab === 'activity' && (
        <>
          <Card style={{ marginBottom: 14 }} className="animate-fade-up d1">
            ...
            }>Commit Trend (12 months)</SectionLabel>
            <ResponsiveContainer width="100%" height={210}>
          ...
          <Card style={{ marginBottom: 14 }} className="animate-fade-up d2">
            ...
            <Heatmap grid={contribGrid} />
            <div style={{ fontSize: 11, color: 'var(--text4)', marginTop: 8, fontStyle: 'italic' }}>
              Heatmap is estimated — GitHub REST API doesn't provide full contribution history
            </div>
          </Card>

          {/* Repo creation timeline */}
          {repoTimeline?.length > 0 && (
            <Card style={{ marginBottom: 14 }} className="animate-fade-up d3">
              <SectionLabel>Repository Creation Timeline</SectionLabel>
              <div style={{ overflowX: 'auto' }}>
                <div style={{ display: 'flex', gap: 0, position: 'relative', paddingBottom: 8, minWidth: 400 }}>
                  <div style={{ position: 'absolute', top: 16, left: 0, right: 0, height: 2, background: 'var(--border)' }} />
                  {repoTimeline.map((r, i) => (
                    <div key={r.name} style={{ flex: 1, minWidth: 60, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}>
                      <div style={{ width: 10, height: 10, borderRadius: '50%', background: r.stars > 0 ? 'var(--br2)' : 'var(--border2)', border: '2px solid var(--surface)', zIndex: 1, flexShrink: 0 }} title={`${r.name} — ${r.created}`} />
                      <div style={{ fontSize: 9, color: 'var(--text4)', textAlign: 'center', writingMode: 'vertical-rl', transform: 'rotate(180deg)', height: 50, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: 14 }} title={r.name}>{r.name}</div>
                      <div style={{ fontSize: 8, color: 'var(--text4)' }}>{r.created.slice(2)}</div>
                    </div>
                  ))}
                </div>
              </div>
              <div style={{ fontSize: 11, color: 'var(--text4)', marginTop: 4, fontStyle: 'italic' }}>Showing last {repoTimeline.length} repos by creation date. Orange dots = have stars.</div>
            </Card>
          )}
                ...
                {activity.map((a, i) => (
                  <div key={i} style={{ display: 'flex', gap: 12, padding: '11px 0', borderBottom: i < activity.length - 1 ? '1px solid var(--border)' : 'none', alignItems: 'flex-start' }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ fontSize: 11, color: 'var(--text4)', marginTop: 3, display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                        {a.repo && <span style={{ fontWeight: 600, color: 'var(--br3)' }}>{a.repo}</span>}
                        <span>{timeAgo(a.time)}</span>
                        <span style={{ padding: '1px 7px', borderRadius: 20, background: 'var(--bg2)', color: ACT_COLORS[a.type] || 'var(--text3)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.04em', fontSize: 10, border: `1px solid ${ACT_COLORS[a.type] || 'var(--border)'}22` }}>{ACT_LABELS[a.type]}</span>
                      </div>
                    </div>
                  </div>
                ))}
