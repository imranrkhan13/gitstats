...
function ProfileCard({ data, cardRef }) {
  ...
  const hasCommits = monthlyCommits?.length > 0
    ...
    <div ref={cardRef} style={{
      ...
      {badges.length > 0 && (
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 16 }}>
          {badges.map(b => (
            <div key={b.label} style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '4px 10px', borderRadius: 20, background: 'rgba(245,200,66,0.1)', border: '1px solid rgba(245,200,66,0.2)' }}>
              <span style={{ fontSize: 12 }}>{b.icon}</span>
              <span style={{ fontSize: 10, fontWeight: 700, color: '#f5c842', letterSpacing: '0.03em' }}>{b.label}</span>
            </div>
          ))}
        </div>
      )}
      ...
      {/* Commit mini-chart — only if real data exists */}
      {hasCommits && (
        <div style={{ marginBottom: 18 }}>
          <div style={{ fontSize: 9, color: 'rgba(255,255,255,0.25)', letterSpacing: '0.07em', textTransform: 'uppercase', marginBottom: 7 }}>
            Recent commits ({monthlyCommits.length} months)
          </div>
          <div style={{ height: 46 }}>
              ...
              <BarChart data={monthlyCommits} margin={{ top: 0, right: 0, bottom: 0, left: 0 }} barCategoryGap="22%">
                <Bar dataKey="commits" radius={[2, 2, 0, 0]}>
                  {monthlyCommits.map((_, i) => (
                    <Cell key={i} fill={i === monthlyCommits.length - 1 ? '#f5c842' : `rgba(255,255,255,${0.06 + (i / monthlyCommits.length) * 0.26})`} />
                  ))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 20 }}>
        ...
        <div style={{ background: 'rgba(245,200,66,0.07)', border: '1px solid rgba(245,200,66,0.14)', borderRadius: 11, padding: '12px 14px' }}>
          <div style={{ fontSize: 9, color: 'rgba(255,255,255,0.3)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 4, display: 'flex', alignItems: 'center', gap: 5 }}>
            <TrophyIcon size={9} color="rgba(255,255,255,0.3)" />Longest streak
          </div>
...
function buildStatsText(data, data2 = null) {
  ...
  if (data2) {
    ...
    return [
      `⚔️ GitHub Battle: @${u.login} vs @${u2.login}`,
      ``,
...
export function ShareCard({ data, data2 = null, show, onClose }) {
  ...
  const handleTwitter = () => {
    const text = isCompare
      ? `⚔️ GitHub battle: @${data.user.login} vs @${data2?.user.login} — who wins? Check it out on ${BRAND.appName}`
      : `📊 My GitHub stats: Score ${data.score}/100, ${fmt(data.totalStars)}⭐, ${data.streak}d streak! Check yours on ${BRAND.appName}`
    const url = isCompare
      ? `${BRAND.websiteUrl}/?user=${data.user.login}&compare=${data2.user.login}`
      : `${BRAND.websiteUrl}/?user=${data.user.login}`
  ...
  const BtnIcon = ({ type }) => {
    if (isRendering && ['download', 'copy'].includes(type)) return <div style={{ width: 14, height: 14, border: '2px solid rgba(255,255,255,0.3)', borderTopColor: '#fff', borderRadius: '50%', animation: 'spin 0.7s linear infinite' }} />
    ...
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(8,3,0,0.9)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 20, backdropFilter: 'blur(14px)' }}>
      <div onClick={e => e.stopPropagation()} style={{ width: '100%', maxWidth: 500, maxHeight: '92vh', overflowY: 'auto', animation: 'scaleIn 0.28s cubic-bezier(0.22,1,0.36,1)' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 8 }}>
          ...
          <button onClick={handleCopyImage} disabled={isRendering} style={{ ...btnBase, ...(status === 'done-copy' ? doneStyle : {}) }}>
            <BtnIcon type="copy" />
